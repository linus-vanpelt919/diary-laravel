@if ($paginator->hasPages())
    ...
    {{-- First Page Link --}}
    <!-- 最初のページへのリンク ページがなかったらリンクを無効に-->
        <li class="p-pagination__page-item {{ $paginator->onFirstPage() ? ' disabled' : '' }}">
    ...
    <!-- 前のページへのリンク -->
        ...
        <!--config('const.PAGINATE.LINK_NUM')でconst.phpに書いたLINK_NUMを呼び出す -->
        ...
        @for ($i = $start_page; $i <= $end_page; $i++)
            @if ($i == $paginator->currentPage())
                <li class="p-pagination__page-item active"><span class="page-link">{{ $i }}</span></li>
            @else
                <li class="p-pagination__page-item"><a class="page-link" href="{{ $paginator->url($i) }}">{{ $i }}</a></li>
        @endif
    @endfor

    {{-- Next Page Link --}}
    <!-- 次のページへのリンク -->
        <li class="p-pagination__page-item {{ $paginator->currentPage() == $paginator->lastPage() ? ' disabled' : '' }}">
